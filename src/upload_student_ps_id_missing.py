...
def format_re_students(df):
        ...
        df = df.rename(columns={'Constituent ID': 'Const_ID',
                                'First Name': 'First_Name',
                                'Last Namee': 'Last_Name',
                                'Primary Education Class of': 'Class_Of'
                                })

        df['Class_Of'] = df['Class_Of'].str.replace('\'', '', regex=True).astype(int).add(2000)
        return df
...
if __name__ == '__main__':
    ...
    re_students = read_file_csv("cs_ps_id_missing.CSV", path_data)

    re_students = format_re_students(re_students)

    print(re_students.to_markdown())
